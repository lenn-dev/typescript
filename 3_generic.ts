...
// 타입스크립트로 하여금 타입을 추정하게끔 만드는 generic을 사용
...
// 이때 함수에서 받는 배열의 타입이 다양할 수 있는데,
// 수많은 경우의 수를 생각하여 배열타입을 일일히 명시해야 하는 번거로움이 있다.
// ex) boolean배열, object, string배열...[string,boolean,number]

// 우리가 흔히 사용하는 타입들(number, string, void , unknown...)은 concrete type 이다
// 여기서는 concrete 타입이 아닌 generic 타입을 받아서 해결할 수 있다.
// generic은 타입의 placeholder 같은 것
// generic 타입은 타입을 추론해 함수를 사용할 수 있게 한다.

// superPrint([1,2,true,false])
// 이러한 배열로 함수를 호출한다면 이에 해당하는 call signiture를 또 써줘야 함
// 모든 타입의 가능성을 조합해서 명시하는 것은 불가능
// generic 타입으로 바꾼다면 쉬워진다.
